...
const VIDEO_URL = "/video/scroll-optimized.mp4";
const TAGLINE = "KENJI & Sarah";
...
export function VideoScroller({ src = VIDEO_URL, duration = 5 }: VideoScrollerProps) {
  ...
  // ── OUR STORY: one-way MotionValues ──────────────────────────────────────────
  // These values ONLY move toward "fully visible". Scrolling back cannot reverse them.
  // This is the definitive fix for the bidirectional useTransform flicker.
  const ourOp     = useMotionValue(0);
  const storyOp   = useMotionValue(0);
  const ourY      = useMotionValue(26);
  const storyY    = useMotionValue(26);
  const ourBlur   = useMotionValue(14);
  const storyBlur = useMotionValue(14);
  const ourFilter   = useTransform(ourBlur,   v => `blur(${v}px)`);
  const storyFilter = useTransform(storyBlur, v => `blur(${v}px)`);
  ...
  useEffect(() => {
    ...
    storyBlur.set(0);
    bottomGlassOp.set(1);
  }, [reducedMotion, ourOp, ourY, ourBlur, storyOp, storyY, storyBlur, bottomGlassOp]);
  ...
  useEffect(() => {
    return scrollYProgress.on("change", (p) => {
      ...
      // "OUR": enters [0.03 → 0.10]
      const op = Math.max(0, Math.min(1, (p - 0.03) / 0.07));
      ourOp.set(Math.max(ourOp.get(), op));
      ourY.set(Math.min(ourY.get(), 26 * (1 - op)));
      ourBlur.set(Math.min(ourBlur.get(), 14 * (1 - op)));

      // "STORY": enters [0.09 → 0.16]
      const sp = Math.max(0, Math.min(1, (p - 0.09) / 0.07));
      storyOp.set(Math.max(storyOp.get(), sp));
      storyY.set(Math.min(storyY.get(), 26 * (1 - sp)));
      storyBlur.set(Math.min(storyBlur.get(), 14 * (1 - sp)));
      ...
      <div className="sticky top-0 h-screen overflow-hidden bg-wedding-obsidian">
        ...
        >
          <h1 className="font-display text-5xl md:text-7xl lg:text-[5.5rem] tracking-[0.18em] uppercase leading-none text-center">
            <motion.span style={{ opacity: ourOp,   y: ourY,   filter: ourFilter,   display: "inline-block" }}>OUR</motion.span>
            {" "}
            <motion.span style={{ opacity: storyOp, y: storyY, filter: storyFilter, display: "inline-block" }}>STORY</motion.span>
          </h1>
        </div>
        ...
        {/* ── OUR STORY completion card (z-40) ──────────────────────────────── */}
        <motion.div
          aria-hidden
